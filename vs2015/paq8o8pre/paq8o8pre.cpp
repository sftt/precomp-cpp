...
int main(int argc, char** argv) {
  ...
  try {
    ...
    if (0 < level) {
        switches.compression_method = 0;
    } else {
        switches.pdf_bmp_mode = false;
        switches.use_mp3 = false;
        switches.compression_otf_thread_count = 1;
    }
    ...
    if (argc<2) {
        ...
        printf(PROG_EXE " v2 archiver (C) 2008, schnaader, KZ et al.\n"
...
#endif
        "To compress:\n"
        "  " PROGNAME " [switches] -level file               (compresses to file." PROGNAME ")\n"
        "  " PROGNAME " [switches] -level archive files...   (creates archive." PROGNAME ")\n"
        ...
        "\n"
        "To extract:\n"
        "  " PROGNAME " [switches] -d dir1/archive." PROGNAME "      (extract to dir1)\n"
        "  " PROGNAME " [switches] -d dir1/archive." PROGNAME " dir2 (extract to dir2)\n"
  ...
  if (0 < level) { // with Encoder
    ...
    // If there is no dir1, then extract to .
      ...
      for (int j = 0; j < files; ++j) {
          // Precompress file
          printf("%s %ld -> ", fname[j], fsize[j]);
          FILE *f = tmpfile();
          {
              char msg[256], name[256];
              int i = 0;
              while (i < 255 && fname[j][i] != 0) name[i] = fname[j][i], i++; name[i] = 0;
              if (!precompress_f(name, f, msg, switches)) {//"~temp.pcf"
                  printf("%s\n", msg);
                  quit();
              }
          }
          //FILE *f = fopen("~temp.pcf", "rb");
          if (!f) perror("tmpfile"), quit();
          fseek(f, 0, SEEK_END);
          long tsize = ftell(f);
          rewind(f);
          //printf("%s %ld -> %ld", fname[j], fsize[j], tsize);

          HANDLE hFile = (HANDLE)_get_osfhandle(fileno(f));
          DWORD dwFileSizeHi = 0;
          DWORD dwFileSizeLo = GetFileSize(hFile, &dwFileSizeHi);
          if (0 != dwFileSizeLo) {
              HANDLE hFileMap = CreateFileMapping(hFile, NULL, PAGE_READONLY, dwFileSizeHi, dwFileSizeLo, NULL);
              if (!hFileMap) perror("CreateFileMapping"), quit();
              // Create a file mapping to get the file name.
              void* pMem = MapViewOfFile(hFileMap, FILE_MAP_READ, 0, 0, tsize);
              if (!pMem) CloseHandle(hFileMap), perror("MapViewOfFile"), quit();

              fwrite(&tsize, 4, 1, archive);
              /*for (int i = 0; i<4; i++) en.compress(0);
              en.compress(4);
              for (int i = 0; i<4; i++) en.compress((tsize >> (8 * i)) & 255);*/
              fwrite(pMem, tsize, 1, archive);
      ...
      for (int i = 0; i<files; ++i) {
          ...
          //for (int i = 0; i<5; i++) en.decompress();
          //for (int i = 0; i<4; i++) tsize += (en.decompress()) << (8 * i);
          fread(&tsize, 4, 1, archive);

          long osize = fsize[i];
          // Test if output file exists.  If so, then skip
          FILE* f = fopen(out.c_str(), "rb");
          if (f) {
              printf("Output file exists. Skipping %s %ld -> ", out.c_str(), osize);
              fseek(archive, tsize, SEEK_CUR);
              printf("not extracted\n");
              fclose(f);
          }
          else
          {
              FILE* f = tmpfile();//fopen("~temp.pcf", "w+b");
              // Decompress
              if (f) {
                  printf("Extracting %s %ld -> ", out.c_str(), tsize);
                  HANDLE hFile = (HANDLE)_get_osfhandle(fileno(f));
                  HANDLE hFileMap = CreateFileMapping(hFile, NULL, PAGE_READWRITE, 0, tsize, NULL);
                  if (!hFileMap) perror("CreateFileMapping"), quit();
                  // Create a file mapping to get the file name.
                  void* pMem = MapViewOfFile(hFileMap, FILE_MAP_READ | FILE_MAP_WRITE, 0, 0, tsize);
                  if (!pMem) CloseHandle(hFileMap), perror("MapViewOfFile"), quit();

                  fread(pMem, tsize, 1, archive);
                  UnmapViewOfFile(pMem);
                  CloseHandle(hFileMap);
                  //fclose(f);

                  // Try creating directories in path
                  String path(out.c_str());
                  for (int i = 0; path[i]; ++i) {
                      if (path[i] == '/' || path[i] == '\\') {
                          char savechar = path[i];
                          path[i] = 0;
                  ...
                  char msg[256], name[256];
                  int i = 0;
                  while (i<255 && out.c_str()[i] != 0) name[i] = out.c_str()[i], i++; name[i] = 0;
                  if (!recompress_f(f, name, msg, switches)) {
                      printf("%s\n", msg);
                      quit();
                  }
                  printf("done        \n");
              ...
              // Can't create, discard data
                  ...
                  perror(out.c_str());
                  printf("Skipping %s %ld -> ", out.c_str(), tsize);
                  fseek(archive, tsize, SEEK_CUR);
                  printf("not extracted\n");
              }
          }
